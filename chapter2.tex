...
Comenzaremos esta sección definiendo qué es un sistema de recomendación y para qué se usan \cite{HandbookRS}.
\begin{quote}
``
Los sistemas de recomendación (SRs) son herramientas software y técnicas que proporcionan sugerencias para que los objetos sean útiles para un usuario. Las sugerencias están relacionadas a varios procesos de toma de decisiones
como pueden ser objetos para comprar, qué música escuchar, o que noticias online podrías leer.
Un ``objeto'' es el término general usado para denotar lo que el sistema le recomienda a los usuarios. Un  SR normalmente se centra en un tipo específico de objeto (por ejemplo, CDs o noticias) 
