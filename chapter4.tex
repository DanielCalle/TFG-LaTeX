...
En backend, una de los arquitecturas de moda hoy en día es el de microservicios\cite{microservices}\cite{microservicesspring}. Este patrón permite la creación 
de un servidor gigante formado por pequeños servicios interrelacionados entre sí. Esto hace más fácil el despliegue y 
...
\begin{enumerate}
    ...
    \item Registrador de servicios (\textbf{Registry}), es el componente que se registran todos los servicios. Para organizarlos y distribuirlos.
\end{enumerate}
Al final se consiguió implementar este patrón mediante \textbf{Eureka} (registrador de servicios) y \textbf{Zuul} (puerta de enlace).
Sin embargo, se rechazó la implementación de este patrón en el proyecto pues nuestro proyecto no era lo bastante grande como tener 
la necesitad de usar este patrón. Se bastaba con solo un simple servidor de API REST.
